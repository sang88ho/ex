...
public class ExchangeFinder {
    ...
    */
    public ExchangeFinder() throws SQLException {
    	...
    	StringBuffer strTemp; // 콘솔 출력을 위한 임시 string
    	System.out.println("ID\t" + "Bank\t" + "Nation\t" + "Code\t" + "TRA_S\t" + "TRA_R\t"
    							  + "CASH_S\t" + "CASH_T\t" + "Stand\t" + "USDRate\t" + "Date");
    	
    	
    	//this.searchBank("woori");
    	//a = this.searchBank("shinhan");    	
    	//a = this.searchDate("2013", "12", "04");
    	strTemp = findMaximum_NationAndDate(true, TransferType.CASH_SEND, "미국", "2013", "12", "05");
    ...
    public StringBuffer findMaximum_NationAndDate (
    		boolean isMaximun, TransferType type, String Nation,
    	...
    	case TRANSFER_SEND:
    		if (isMaximun)
    			...
    			result = state.executeQuery	    		
    			("select * from exchange where nation = \'" + Nation + '\'' + "order by transferSend");
    		break;
    	case TRANSFER_RECEIVE:
    		if (isMaximun)
    			...
    			result = state.executeQuery
	    		("select * from exchange where nation = \'" + Nation + '\'' + "order by transferReceive");	    		
    		break;
    	case CASH_SEND:
    		if (isMaximun)
    			result = state.executeQuery
	    		...
	    		result = state.executeQuery
	    		("select * from exchange where nation = \'" + Nation + '\'' + "order by cashSend ");
    		break;
    	case CASH_RECEIVE:
    		if (isMaximun)
	    		result = state.executeQuery
    			...
    			result = state.executeQuery
	    		("select * from exchange where nation = \'" + Nation + '\'' + "order by CashReceive ");	    		
    		break;
    	}
